...
class DynamicTopo(Topo):
    ...
    def __init__(self, n=1, delay='1ms', z=1, bw=1, cpu=.1, max_queue_size=None, **params):
        ...
        hostConfig = {'cpu': cpu}
        linkConfig = {'bw': args.bw, 'delay': args.delay, 'loss': 0, 'max_queue_size': max_queue_size }
        
        for x in range(1, args.z + 1):
             self.addSwitch('s%d' % (x))
             if x > 1:
...
def main():
    ...
    for x in range(1, args.z + 1):
         net.get('s%d' % x).cmd('ovs-vsctl set bridge s%d stp-enable=true' % (x))
